from store.db_model import Event
import pytest
...
def request(event_json, response):
    {
        k: event_json[k]
        for k in event_json
        if k in response.json() and event_json[k] == response.json()[k]
    }


@pytest.mark.parametrize("param", [1, 7])
...
def test_upload(api_client: TestClient, populate_event_table):
    ...
    response = api_client.post("/upload", json=j)
    assert response.status_code == 200
